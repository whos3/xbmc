  ...
  : CAddon(addonInfo, ADDON_MEDIAIMPORTER)
{
  ...
  m_protocol = addonTypeInfo->GetValue("@protocol").asString();
  ...
  m_providerSettingsPath = addonTypeInfo->GetValue("providersettings").asString();
  m_importSettingsPath = addonTypeInfo->GetValue("importsettings").asString();
}

std::string CMediaImporter::Protocol() const
{
  if (!m_protocol.empty())
    return m_protocol;
