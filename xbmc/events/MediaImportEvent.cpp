...
bool CMediaImportSourceEvent::Execute() const
{
  ...
  params.push_back(StringUtils::Format("import://%s/", CURL::Encode(m_source.GetIdentifier()).c_str()));
...
CMediaImportEvent::CMediaImportEvent(const CMediaImport& import, const CVariant& description, bool removed, EventLevel level /* = EventLevel::Information */)
  : CUniqueEvent(StringUtils::Format(g_localizeStrings.Get(39565).c_str(), import.GetSource().GetFriendlyName().c_str(), CMediaTypes::ToLabel(import.GetMediaTypes()).c_str()),
...
bool CMediaImportEvent::Execute() const
{
  ...
  return CServiceBroker::GetMediaImportManager().Import(m_import.GetPath(), m_import.GetMediaTypes());
}
