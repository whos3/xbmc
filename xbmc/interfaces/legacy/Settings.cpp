...
namespace XBMCAddon
{
  namespace xbmcaddon
  {
    ...
    bool GetSettingValue(std::shared_ptr<CSettingsBase> settings, const std::string& key, typename TSetting::Value& value)
    {
    ...
    template<class TSetting>
    bool GetSettingValueList(std::shared_ptr<CSettingsBase> settings, const std::string& key, std::function<typename TSetting::Value(CVariant)> transform, std::vector<typename TSetting::Value>& values)
    {
      if (key.empty() || !settings->IsLoaded())
        return false;

      auto setting = settings->GetSetting(key);
      if (setting == nullptr || setting->GetType() != SettingType::List || std::static_pointer_cast<CSettingList>(setting)->GetElementType() != TSetting::Type())
        return false;
    ...
    template<class TSetting>
    bool SetSettingValue(std::shared_ptr<CSettingsBase> settings, const std::string& key, typename TSetting::Value value)
    {
      if (key.empty() || !settings->IsLoaded())
        return false;
    ...
    template<class TSetting>
    bool SetSettingValueList(std::shared_ptr<CSettingsBase> settings, const std::string& key, const std::vector<typename TSetting::Value>& values)
    {
      if (key.empty() || !settings->IsLoaded())
        return false;
      ...
      std::transform(values.begin(), values.end(), std::back_inserter(variantValues), [](typename TSetting::Value value) { return CVariant(value); });
    ...
    void Settings::setBoolList(const char* id, const std::vector<bool>& values) throw(XBMCAddon::WrongTypeException)
    {
    ...
    void Settings::setIntList(const char* id, const std::vector<int>& values) throw(XBMCAddon::WrongTypeException)
    {
    ...
    void Settings::setNumberList(const char* id, const std::vector<double>& values) throw(XBMCAddon::WrongTypeException)
    {
    ...
    void Settings::setStringList(const char* id, const std::vector<String>& values) throw(XBMCAddon::WrongTypeException)
    {
    ...
    bool Settings::registerActionCallback(const String& settingId, const String& callback) throw(SettingCallbacksNotSupportedException)
    {
      if (m_callbackExecutor == nullptr)
        throw SettingCallbacksNotSupportedException("Add-on %s does not support setting callbacks", m_addonId.c_str());
    ...
    bool Settings::registerOptionsFillerCallback(const String& settingId, const String& callback) throw(SettingCallbacksNotSupportedException)
    {
      if (m_callbackExecutor == nullptr)
        throw SettingCallbacksNotSupportedException("Add-on %s does not support setting callbacks", m_addonId.c_str());
      ...
      case SettingType::Integer:
        std::static_pointer_cast<CSettingInt>(setting)->SetOptionsFiller(IntegerSettingOptionsFiller, this);
        break;

      case SettingType::String:
        std::static_pointer_cast<CSettingString>(setting)->SetOptionsFiller(StringSettingOptionsFiller, this);
        break;
    ...
    bool Settings::setIntegerOptions(const String& settingId,
      const std::vector<Tuple<String, int>>& options)
      throw(SettingCallbacksNotSupportedException)
    {
      if (m_callbackExecutor == nullptr)
        throw SettingCallbacksNotSupportedException("Add-on %s does not support setting callbacks", m_addonId.c_str());
    ...
    bool Settings::setStringOptions(const String& settingId,
      const std::vector<Tuple<String, String>>& options)
      throw(SettingCallbacksNotSupportedException)
    {
      if (m_callbackExecutor == nullptr)
        throw SettingCallbacksNotSupportedException("Add-on %s does not support setting callbacks", m_addonId.c_str());
    ...
    void Settings::IntegerSettingOptionsFiller(std::shared_ptr<const CSetting> setting, IntegerSettingOptions& list, int& current, void* data)
    {
      ...
      settings->m_callbackExecutor->OnIntegerSettingOptionsFiller(setting, callback->second, list, current, settings->m_callbackData);
    }

    void Settings::StringSettingOptionsFiller(std::shared_ptr<const CSetting> setting, StringSettingOptions& list, std::string& current, void* data)
    {
      if (setting == nullptr || data == nullptr)
        return;
      ...
      settings->m_callbackExecutor->OnStringSettingOptionsFiller(setting, callback->second, list, current, settings->m_callbackData);
    }
#endif
  }
}
