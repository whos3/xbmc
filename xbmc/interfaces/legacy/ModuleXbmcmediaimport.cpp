...
namespace XBMCAddon
{
  namespace xbmcmediaimport
  {
    bool addProvider(const MediaProvider* provider)
    {
      ...
      const auto importerId = CAddonMediaImporter::GetImporterId(LanguageHook::GetLanguageHook()->GetAddonId());
    ...
    {
      CAddonMediaImporter::SetCanUpdateResumePositionOnProvider(handle, canUpdateResumePositionOnProvider);
    }
    ...
    void addImportItem(int handle, const XBMCAddon::xbmcgui::ListItem* item, const String& mediaType,
      int changesetType /* = MediaImportChangesetTypeNone */)
    {
      AddonClass::Ref<xbmcgui::ListItem> pItem(item);

      CAddonMediaImporter::AddImportItem(handle, pItem->item, mediaType, static_cast<MediaImportChangesetType>(changesetType));
    }

    void addImportItems(int handle, const std::vector<XBMCAddon::xbmcgui::ListItem*>& items, const String& mediaType,
      int changesetType /* = MediaImportChangesetTypeNone */)
    {
      ...
      CAddonMediaImporter::AddImportItems(handle, fileItems, mediaType, static_cast<MediaImportChangesetType>(changesetType));
    }
    ...
    bool changeImportedItems(const MediaImport* import, const std::vector<ChangesetListItem>& changedItems)
    {
      ...
      for (const auto& changedItem : changedItems)
      {
        ...
        changesetItems.emplace_back(static_cast<MediaImportChangesetType>(changedItem.first()), pChangedItem->item);
      }

      return CServiceBroker::GetMediaImportManager().ChangeImportedItems(*import->import, changesetItems);
    }
    ...
    std::vector<XBMCAddon::xbmcgui::ListItem*> getImportedItemsByProvider(const MediaProvider* provider)
    {
      ...
      const auto items = CServiceBroker::GetMediaImportManager().GetImportedItemsBySource(*provider->source);
      std::vector<XBMCAddon::xbmcgui::ListItem*> importedItems;
      for (const auto& item : items)
        importedItems.push_back(new XBMCAddon::xbmcgui::ListItem(item));
    ...
    std::vector<XBMCAddon::xbmcgui::ListItem*> getImportedItemsByImport(const MediaImport* import)
    {
      ...
      const auto items = CServiceBroker::GetMediaImportManager().GetImportedItemsByImport(*import->import);
      std::vector<XBMCAddon::xbmcgui::ListItem*> importedItems;
      for (const auto& item : items)
        importedItems.push_back(new XBMCAddon::xbmcgui::ListItem(item));
