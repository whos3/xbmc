...
namespace XBMCAddon
{
  namespace xbmcmediaimport
  {
    ...
    MediaProvider::MediaProvider(MediaImportSourcePtr source, const std::string& addonId /* = "" */,
      CAddonMediaImporter* addonMediaImporter /* = nullptr */)
      : source(source)
      , m_addonId(addonId)
    ...
    MediaProvider::MediaProvider(const CMediaImportSource& source, const std::string& addonId /* = "" */,
      CAddonMediaImporter* addonMediaImporter /* = nullptr */)
      : MediaProvider(std::make_shared<CMediaImportSource>(source), addonId, addonMediaImporter)
    { }
#endif

    MediaProvider::MediaProvider(const String& identifier, const String& basePath /* = emptyString */,
      const String& friendlyName /* = emptyString */, const String& iconUrl /* = emptyString */,
      const MediaTypes& mediaTypes /* = {} */, const String& lastSynced /* = emptyString */,
      int handle /* = -1 */)
      : source(std::make_shared<CMediaImportSource>(identifier, basePath, friendlyName, iconUrl, mediaTypes))
    ...
    std::vector<MediaImport*> MediaProvider::getImports() const {
      const auto imports = CServiceBroker::GetMediaImportManager().GetImportsBySource(source->GetIdentifier());
