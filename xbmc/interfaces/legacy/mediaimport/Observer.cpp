...
namespace XBMCAddon
{
  namespace xbmcmediaimport
  {
    ...
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaProvider*>(this, &Observer::onProviderAdded, new MediaProvider(source, m_importerId)));
    }

    void Observer::OnSourceUpdated(const CMediaImportSource& source)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaProvider*>(this, &Observer::onProviderUpdated, new MediaProvider(source, m_importerId)));
    }

    void Observer::OnSourceRemoved(const CMediaImportSource& source)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaProvider*>(this, &Observer::onProviderRemoved, new MediaProvider(source, m_importerId)));
    }

    void Observer::OnSourceActivated(const CMediaImportSource& source)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaProvider*>(this, &Observer::onProviderActivated, new MediaProvider(source, m_importerId)));
    }

    void Observer::OnSourceDeactivated(const CMediaImportSource& source)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaProvider*>(this, &Observer::onProviderDeactivated, new MediaProvider(source, m_importerId)));
    }

    void Observer::OnImportAdded(const CMediaImport& import)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaImport*>(this, &Observer::onImportAdded, new MediaImport(import, m_importerId)));
    }

    void Observer::OnImportUpdated(const CMediaImport& import)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaImport*>(this, &Observer::onImportUpdated, new MediaImport(import, m_importerId)));
    }

    void Observer::OnImportRemoved(const CMediaImport& import)
    {
      XBMC_TRACE;
      invokeCallback(new CallbackFunction<Observer, MediaImport*>(this, &Observer::onImportRemoved, new MediaImport(import, m_importerId)));
    }
