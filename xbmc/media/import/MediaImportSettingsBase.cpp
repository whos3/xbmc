...
CMediaImportSettingsBase::CMediaImportSettingsBase(const std::string& settingValues /* = "" */)
  : CSettingsBase(), m_settingValues(settingValues), m_logger(CServiceBroker::GetLogging().GetLogger("CMediaImportSettingsBase"))
{
}
...
bool CMediaImportSettingsBase::Load()
{
  ...
  if (!Initialize())
  {
    m_logger->error("failed to initialize settings");
    return false;
  }
  ...
  if (!m_settingValues.empty())
  {
    ...
    {
      m_logger->error("error loading setting values, Line {}\n{}", xmlValues.ErrorRow(),
                      xmlValues.ErrorDesc());
      Uninitialize();
      return false;
    }
    ...
    {
      m_logger->error("failed to load setting values");
      Uninitialize();
      return false;
    }
  }
...
bool CMediaImportSettingsBase::InitializeDefinitions()
{
  ...
  for (const auto& settingDefinition : m_settingDefinitions)
  {
    ...
    {
      m_logger->error("error loading settings definition, Line {}\n{}", xmlDefinition.ErrorRow(),
