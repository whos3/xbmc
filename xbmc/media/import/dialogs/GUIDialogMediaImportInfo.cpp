...
void CGUIDialogMediaImportInfo::Save()
{
  ...
  CMediaImportManager::GetInstance().UpdateImport(*m_import);
}
...
bool CGUIDialogMediaImportInfo::SetMediaImport(const CFileItemPtr &item)
{
  ...
  m_import = new CMediaImport(importPath, mediaTypes);
...
void CGUIDialogMediaImportInfo::SetupMediaTypes()
{
  ...
  // get all media types available for ths media source
  const MediaTypes &availableMediaTypes = m_source->GetAvailableMediaTypes();
  if (availableMediaTypes.empty())
    return;

  // group the media types together
  std::vector<GroupedMediaTypes> groupedMediaTypes = CMediaImportManager::GetInstance().GetSupportedMediaTypesGrouped(availableMediaTypes);
  if (groupedMediaTypes.empty())
    return;

  // get all existing imports of the media source
  std::vector<CMediaImport> imports = CMediaImportManager::GetInstance().GetImportsBySource(m_source->GetIdentifier());

  for (const auto& itMediaTypes : groupedMediaTypes)
  {
    CFileItemPtr item(new CFileItem(MediaTypesToLabel(itMediaTypes)));

    for (std::vector<CMediaImport>::iterator itImport = imports.begin(); itImport != imports.end(); ++itImport)
    {
      if (itMediaTypes == itImport->GetMediaTypes())
      {
        item->Select(true);
