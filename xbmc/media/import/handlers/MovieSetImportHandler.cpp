...
CFileItemPtr CMovieSetImportHandler::FindMatchingLocalItem(
  const CMediaImport& import, const CFileItem* item, const std::vector<CFileItemPtr>& localItems) const
{
...
bool CMovieSetImportHandler::AddImportedItem(const CMediaImport &import, CFileItem* item)
{
  ...
  item->GetVideoInfoTag()->m_iDbId = m_db.SetDetailsForMovieSet(*(item->GetVideoInfoTag()), item->GetArt());
...
bool CMovieSetImportHandler::UpdateImportedItem(const CMediaImport &import, CFileItem* item)
{
  ...
  if (m_db.SetDetailsForMovieSet(*(item->GetVideoInfoTag()), item->GetArt(), item->GetVideoInfoTag()->m_iDbId) <= 0)
    return false;
...
bool CMovieSetImportHandler::GetLocalItems(CVideoDatabase &videodb, const CMediaImport &import, std::vector<CFileItemPtr>& items) const
{
  CFileItemList movieSets;
  if (!videodb.GetSetsByWhere("videodb://movies/sets/?imported&import=" + CURL::Encode(import.GetPath()), CDatabase::Filter(), movieSets, false))
    return false;
