...
public:
  ...
  MediaImportChangesetType DetermineChangeset(const CMediaImport &import, const CFileItem* item, const CFileItemPtr& localItem) override;
  void PrepareImportedItem(const CMediaImport& import, CFileItem* item, const CFileItemPtr& localItem) const override;
...
protected:
  ...
  bool Compare(const CFileItem *originalItem, const CFileItem *newItem, bool allMetadata = true, bool playbackMetadata = true) const;

  static void RemoveAutoArtwork(CGUIListItem::ArtMap& artwork, const std::set<std::string>& parentPrefixes);
