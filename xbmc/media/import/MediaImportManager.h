...
public:
   ...
   */
  void RegisterSource(const std::string& sourceID, const std::string& friendlyName, const std::set<MediaType>& mediaTypes = std::set<MediaType>());
  ...
  /*!
   * \brief Import media items of the given media types from the given path
   *
   * \param sourceID Unique identifier of the source
   * \param path Path from where media items will be imported
   * \param mediaTypes list of media types to import
   * \return True if the import of media items has been started, false otherwise
   */
  bool Import(const std::string& sourceID, const std::string& path, const std::set<MediaType>& mediaTypes);
   ...
   * \return TODO
   */
  bool Import(const std::string& sourceID, const MediaType& mediaType, const CFileItemPtr& item);
  /*!
   * \brief TODO
   *
   * \param sourceID Unique identifier of the source
   ...
   * \return TODO
   */
  bool Import(const std::string& sourceID, const MediaType& mediaType, const CFileItemList& items);
...
private:
  ...
  bool FindSource(const std::string &sourceID, CMediaImportSource &source);

  bool AddImport(const CMediaImport &import);
  bool FindImport(const std::string &path, CMediaImport &import);

  void AddTaskProcessorJob(const std::string &path, CJob *job);

  bool ImportSource(const std::string &sourceID);
  bool ImportNew(const CMediaImportSource &source, const std::string& path, const std::set<MediaType>& mediaTypes);
  void Import(const CMediaImport &import);

  void SetImportItemsEnabled(const std::string& sourceID, bool enabled);
  ...
  std::map<std::string, CJob*> m_jobMap;
};
