...
bool CMusicVideoImportHandler::HandleImportedItems(CVideoDatabase &videodb, const CMediaImport &import, const CFileItemList &items, IMediaImportTask *task)
{
  ...
  task->SetProgressTitle("Importing musicvideos from " + import.GetSource().GetFriendlyName()); // TODO: localization
  task->SetProgressText("");

  CFileItemList storedItems;
  videodb.GetMusicVideosByWhere("videodb://musicvideos/titles/", GetFilter(import), storedItems, true, SortDescription(), true);
