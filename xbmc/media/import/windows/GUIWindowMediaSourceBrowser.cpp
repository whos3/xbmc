...
std::string CGUIWindowMediaSourceBrowser::ShowAndGetImporterToLookup(const std::vector<MediaImporterFactoryConstPtr>& importers)
{
  if (importers.empty())
    return "";
  ...
  {
    CFileItem item(importer->GetProtocol());
    item.SetPath(importer->GetIdentification());  // abuste the importer's identification
...
bool CGUIWindowMediaSourceBrowser::AddNewSource(CFileItemPtr newImportItem, const CURL& url)
{
  ...
  auto importers = mediaImportManager.GetImporters();
  importers.erase(std::remove_if(importers.begin(), importers.end(),
    [](const MediaImporterFactoryConstPtr& importer) { return !importer->CreateImporter()->CanLookupSource(); }), importers.end());
  ...
  const auto selectedImporter = std::find_if(importers.begin(), importers.end(),
    [importerId](const MediaImporterFactoryConstPtr& importer) { return importer->GetIdentification() == importerId; });
  ...
  if (!mediaImportManager.DiscoverSource(importerId, discoveredSource))
  {
    ...
    KODI::MESSAGING::HELPERS::ShowOKDialogText(CVariant{ (*selectedImporter)->GetProtocol() }, CVariant{ g_localizeStrings.Get(39618) });
    return false;
  }
  ...
  if (mediaImportManager.GetSource(discoveredSource.GetIdentifier(), existingSource))
  {
    ...
    KODI::MESSAGING::HELPERS::ShowOKDialogText(
      CVariant{ (*selectedImporter)->GetProtocol() },
