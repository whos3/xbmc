...
std::set<MediaType> CGUIWindowMediaSourceBrowser::ShowAndGetMediaTypesToImport(const CMediaImportSource &source)
{
  ...
  if (pDialogSelect != NULL)
  {
    ...
    if (pDialogSelect->IsConfirmed() && pDialogSelect->GetSelectedItem() != NULL)
    {
...
bool CGUIWindowMediaSourceBrowser::OnSourceDelete(const CFileItemPtr &item)
{
  ...
  if (progress != NULL)
  {
    progress->SetHeading(StringUtils::Format("Removing %s...", item->GetLabel().c_str())); // TODO: localization
