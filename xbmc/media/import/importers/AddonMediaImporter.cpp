...
{
  // prepare the provider settings in case there's a specific XML file for it
  if (!PrepareProviderSettings(m_addonId, source.Settings()))
    return false;
...
{
  // prepare the import settings in case there's a specific XML file for it
  if (!PrepareImportSettings(m_addonId, import.Settings()))
    return false;
...
bool CAddonMediaImporter::PrepareProviderSettings(const std::string& addonId, MediaImportSettingsBasePtr settings)
{
  ...
  if (LoadSettingsFromFile(addonId, settings, importer->ProviderSettingsPath()))
    return true;

  CLog::Log(LOGWARNING, "CAddonMediaImporter[{}]: failed to load provider settings from {}",
    addonId, importer->ProviderSettingsPath());
...
bool CAddonMediaImporter::PrepareImportSettings(const std::string& addonId, MediaImportSettingsBasePtr settings)
{
  ...
  if (LoadSettingsFromFile(addonId, settings, importer->ImportSettingsPath()))
    return true;

  CLog::Log(LOGWARNING, "CAddonMediaImporter[{}]: failed to load import settings from {}",
    addonId, importer->ImportSettingsPath());
...
std::string CAddonMediaImporterFactory::GetProtocol() const
{
  const auto addon = GetAddon();
  if (addon == nullptr)
    return "";

  return addon->Protocol();
}
