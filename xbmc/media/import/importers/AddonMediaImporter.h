...
protected:
  explicit CAddonMediaImporterBaseInternal(const std::string& addonId);
...
class CAddonMediaImporterBase : public virtual IMediaImporterBase, public CAddonMediaImporterBaseInternal
{
public:
...
protected:
  explicit CAddonMediaImporterBase(const std::string& addonId);
};

class CAddonMediaImporterDiscoverer : public IMediaImporterDiscoverer, public CAddonMediaImporterBase
{
public:
...
class CAddonMediaImporter : public CAddonMediaImporterBase, public CBaseMediaImporter, public ADDON::IAddonSettingsCallbackExecutor
{
public:
  using HandleType = CAddonMediaImporterExecutor::HandleType;
  ...
  void OnSettingAction(std::shared_ptr<const CSetting> setting, const std::string& callback, void* data) override;

  void OnIntegerSettingOptionsFiller(std::shared_ptr<const CSetting> setting,
    const std::string& callback, IntegerSettingOptions& list, int& current, void* data) override;
  bool SetIntegerSettingOptions(const std::string& settingId,
    const IntegerSettingOptions& list, void* data) override;

  void OnStringSettingOptionsFiller(std::shared_ptr<const CSetting> setting,
    const std::string& callback, StringSettingOptions& list, std::string& current, void* data) override;
  bool SetStringSettingOptions(const std::string& settingId,
    const StringSettingOptions& list, void* data) override;;

  // helper methods for add-on scripts
  static bool PrepareProviderSettings(const std::string& addonId, MediaImportSettingsBasePtr settings);
  static bool PrepareProviderSettings(std::shared_ptr<ADDON::CMediaImporter> addon, MediaImportSettingsBasePtr settings);
  static bool PrepareImportSettings(const std::string& addonId, MediaImportSettingsBasePtr settings);
  static bool PrepareImportSettings(std::shared_ptr<ADDON::CMediaImporter> addon, MediaImportSettingsBasePtr settings);

  // methods to interact with the add-on script(s)
  static std::string GetImporterId(const std::string& addonId);

  static void SetDiscoveredProviderDetails(HandleType handle, bool providerDiscovered,
  ...
  static void SetCanImport(HandleType handle, bool canImport) throw(InvalidAddonMediaImporterHandleException);
  static void SetProviderReady(HandleType handle, bool sourceReady) throw(InvalidAddonMediaImporterHandleException);
  static void SetImportReady(HandleType handle, bool importReady) throw(InvalidAddonMediaImporterHandleException);
  static void SetCanUpdateMetadataOnProvider(HandleType handle, bool canUpdateMetadataOnSource) throw(InvalidAddonMediaImporterHandleException);
  static void SetCanUpdatePlaycountOnProvider(HandleType handle, bool canUpdatePlaycountOnSource) throw(InvalidAddonMediaImporterHandleException);
  static void SetCanUpdateLastPlayedOnProvider(HandleType handle, bool canUpdateLastPlayedOnSource) throw(InvalidAddonMediaImporterHandleException);
  static void SetCanUpdateResumePositionOnProvider(HandleType handle, bool canUpdateResumePositionOnSource) throw(InvalidAddonMediaImporterHandleException);

  // methods concerning the current state and progress of the add-on script(s)
  static bool ShouldCancel(HandleType handle, unsigned int progress, unsigned int total) throw(InvalidAddonMediaImporterHandleException);
  static void SetProgressStatus(HandleType handle, const std::string& status) throw(InvalidAddonMediaImporterHandleException);

  // general methods to interact with the add-on script(s)
  static CAddonMediaImporter* GetImporter(HandleType handle) throw(InvalidAddonMediaImporterHandleException);
  static MediaImportSourcePtr GetMediaProvider(HandleType handle) throw(InvalidAddonMediaImporterHandleException);
  static MediaImportPtr GetMediaImport(HandleType handle) throw(InvalidAddonMediaImporterHandleException);

  // methods to interact with the add-on script(s) executing Import()
  static std::vector<CFileItemPtr> GetImportedItems(HandleType handle, const MediaType& mediaType) throw(InvalidAddonMediaImporterHandleException);
  static void AddImportItem(HandleType handle, const CFileItemPtr& item, const MediaType& mediaType,
    MediaImportChangesetType changesetType = MediaImportChangesetTypeNone) throw(InvalidAddonMediaImporterHandleException);
  static void AddImportItems(HandleType handle, const std::vector<CFileItemPtr>& items, const MediaType& mediaType,
    MediaImportChangesetType changesetType = MediaImportChangesetTypeNone) throw(InvalidAddonMediaImporterHandleException);
  static void FinishImport(HandleType handle, bool isChangeset = false) throw(InvalidAddonMediaImporterHandleException);

  // methods to interact with the add-on script(s) executing UpdateOnSource()
  static CFileItemPtr GetUpdatedItem(HandleType handle) throw(InvalidAddonMediaImporterHandleException);
  static void FinishUpdateOnProvider(HandleType handle) throw(InvalidAddonMediaImporterHandleException);
...
private:
  ...
  static bool LoadSettingsFromFile(const std::string& addonId, MediaImportSettingsBasePtr settings, const std::string& settingDefinitionsFile);

  static void SetSuccess(HandleType handle, bool success, CAddonMediaImporterExecutor::Action action) throw(InvalidAddonMediaImporterHandleException);
...
class CAddonMediaImporterObserver : public IMediaImporterObserver, public CAddonMediaImporterBaseInternal
{
public:
...
class CAddonMediaImporterFactory : public IMediaImporterFactory, protected CAddonMediaImporterBaseInternal
{
public:
