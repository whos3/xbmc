...
bool CMediaImportSynchronisationTask::DoWork()
{
  ...
  {
    CLog::Log(LOGINFO, "CMediaImportSynchronisationTask: failed to initialize synchronisation of imported {} items from {}",
      m_importHandler->GetMediaType(), m_import.GetSource());
    return false;
  }

  // prepare the progress bar
  PrepareProgressBarHandle(StringUtils::Format(g_localizeStrings.Get(39561).c_str(),
  ...
  CLog::Log(LOGINFO, "CMediaImportSynchronisationTask: handling {} imported {} items from {}",
    m_items.size(), m_importHandler->GetMediaType(), m_import.GetSource());
  ...
  for (const auto& item : m_items)
  {
    ...
    switch (item.first)
    {
      case MediaImportChangesetTypeAdded:
        SetProgressText(StringUtils::Format(g_localizeStrings.Get(39562).c_str(), itemLabel.c_str()));
        m_importHandler->AddImportedItem(m_import, item.second.get());
        break;

      case MediaImportChangesetTypeChanged:
        SetProgressText(StringUtils::Format(g_localizeStrings.Get(39563).c_str(), itemLabel.c_str()));
        m_importHandler->UpdateImportedItem(m_import, item.second.get());
        break;

      case MediaImportChangesetTypeRemoved:
        SetProgressText(StringUtils::Format(g_localizeStrings.Get(39564).c_str(), itemLabel.c_str()));
      ...
      default:
        CLog::Log(LOGWARNING, "CMediaImportSynchronisationTask: ignoring imported item with unknown changeset type {}", item.first);
        break;
    }
  ...
  {
    CLog::Log(LOGINFO, "CMediaImportSynchronisationTask: failed to finalize synchronisation of imported {} items from {}",
      m_importHandler->GetMediaType(), m_import.GetSource());
    return false;
  }
