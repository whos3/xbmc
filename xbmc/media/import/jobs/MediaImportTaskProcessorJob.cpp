...
{
  if (m_hasProgress && CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_VIDEOLIBRARY_BACKGROUNDUPDATE))
    m_hasProgress = false;
}
...
CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::Import(const CMediaImport& import, bool automatically, const IMediaImporterManager* importerManager, const IMediaImportHandlerManager* importHandlerManager, IMediaImportTaskCallback* callback)
{
  if (importerManager == nullptr)
  {
    CLog::Log(LOGERROR, "CMediaImportTaskProcessorJob: invalid media importer manager implementation");
    return nullptr;
  }

  if (importHandlerManager == nullptr)
  {
    CLog::Log(LOGERROR, "CMediaImportTaskProcessorJob: invalid media import handler manager implementation");
    return nullptr;
  }

  CMediaImport tmpImport = import;
  if (automatically && tmpImport.Settings()->Load() && tmpImport.Settings()->GetImportTrigger() != MediaImportTrigger::Auto)
  {
    CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: automatic import of items from {} is disabled", import);
    return nullptr;
  }

  auto processorJob = new CMediaImportTaskProcessorJob(import.GetSource().GetIdentifier(), importerManager, importHandlerManager, callback, true);
  if (!processorJob->AddImport(import, {}))
  {
    CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: failed to import items from {}", import);
    return nullptr;
  }
...
CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::ChangeImportedItems(const CMediaImport& import, const ChangesetItems &items, const IMediaImportHandlerManager* importHandlerManager, IMediaImportTaskCallback* callback)
{
  if (importHandlerManager == nullptr)
  {
    CLog::Log(LOGERROR, "CMediaImportTaskProcessorJob: invalid media import handler manager implementation");
  ...
  for (const auto& mediaType : import.GetMediaTypes())
  {
    ...
    mediaTypeDataMap.insert(std::make_pair(mediaType, MediaImportTaskData::MediaTypeTaskData{ mediaType, importer }));
  }
  ...
  auto processorJob = new CMediaImportTaskProcessorJob(import.GetSource().GetIdentifier(), nullptr, importHandlerManager, callback, false);
  processorJob->m_importTaskData.emplace(std::make_pair(import.GetPath(), import.GetMediaTypes()), importTaskData);
...
CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::UpdateImportedItemOnSource(const CMediaImport& import, const CFileItem& item, const IMediaImporterManager* importerManager, IMediaImportTaskCallback* callback)
{
  if (importerManager == nullptr)
  {
    CLog::Log(LOGERROR, "CMediaImportTaskProcessorJob: invalid media importer manager implementation");
  ...
  auto processorJob = new CMediaImportTaskProcessorJob(import.GetSource().GetIdentifier(), importerManager, nullptr, callback, false);
...
CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::Cleanup(const CMediaImportSource& source, const std::vector<CMediaImport>& imports, const IMediaImportHandlerManager* importHandlerManager, IMediaImportTaskCallback* callback)
{
  if (importHandlerManager == nullptr)
  {
    CLog::Log(LOGERROR, "CMediaImportTaskProcessorJob: invalid media import handler manager implementation");
  ...
  auto processorJob = new CMediaImportTaskProcessorJob(source.GetIdentifier(), nullptr, importHandlerManager, callback, true);
  ...
  for (const auto& import : imports)
  {
    ...
    {
      CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: failed to cleanup imported items from {}", import);
      continue;
    }
...
CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::Cleanup(const CMediaImport& import, const IMediaImportHandlerManager* importHandlerManager, IMediaImportTaskCallback* callback)
{
  return Cleanup(import.GetSource(), { import }, importHandlerManager, callback);
}

CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::Remove(const CMediaImportSource& source, const std::vector<CMediaImport>& imports, const IMediaImportHandlerManager* importHandlerManager, IMediaImportTaskCallback* callback)
{
  if (importHandlerManager == nullptr)
  {
    CLog::Log(LOGERROR, "CMediaImportTaskProcessorJob: invalid media import handler manager implementation");
  ...
  for (const auto& import : imports)
  {
    if (!processorJob->AddImport(import, tasksToBeProcessed))
    {
      CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: failed to remove imported items from {}", import);
      continue;
    }
  }
...
CMediaImportTaskProcessorJob* CMediaImportTaskProcessorJob::Remove(const CMediaImport& import, const IMediaImportHandlerManager* importHandlerManager, IMediaImportTaskCallback* callback)
{
...
CGUIDialogProgressBarHandle* CMediaImportTaskProcessorJob::GetProgressBarHandle(const std::string& title /* = "" */)
{
  ...
  if (m_progress == nullptr)
  {
    auto dialog = static_cast<CGUIDialogExtendedProgressBar*>(CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_EXT_PROGRESS));
    if (dialog != nullptr)
...
bool CMediaImportTaskProcessorJob::operator==(const CJob* job) const
{
  ...
  bool equalBase = m_path == rjob->m_path &&
...
bool CMediaImportTaskProcessorJob::ProcessTask()
{
  ...
  default:
    CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: unknown import task type {}", static_cast<int>(currentTaskType));
    return false;
  }
...
bool CMediaImportTaskProcessorJob::ProcessTask(IMediaImportTask* task)
{
  ...
  CLog::Log(LOGDEBUG, "CMediaImportTaskProcessorJob: processing {} task from {}...",
  ...
  perf.Stop();
  CLog::Log(LOGDEBUG, "CMediaImportTaskProcessorJob: processing {} task from {} took {} s",
    MediaImportTaskTypes::ToString(task->GetType()), import, perf.GetDurationInSeconds());
...
void CMediaImportTaskProcessorJob::ProcessLocalItemsRetrievalTasks()
{
  for (auto&& taskData = m_importTaskData.begin(); taskData != m_importTaskData.end(); )
  {
    ...
    CMediaImportLocalItemsRetrievalTask* localItemsRetrievalTask = new CMediaImportLocalItemsRetrievalTask(import, mediaImportHandlers);

    // if processing the task failed remove the import (no cleanup needed)
    CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: starting local items retrieval task for items from {}...", import);
    if (!ProcessTask(localItemsRetrievalTask))
    {
      CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: local items retrieval task for items from {} failed", import);
...
void CMediaImportTaskProcessorJob::ProcessImportItemsRetrievalTasks()
{
  ...
  {
    const CMediaImport& import = taskData->second.m_import;
    CMediaImportImportItemsRetrievalTask* importItemsRetrievalTask = new CMediaImportImportItemsRetrievalTask(import, m_importerManager);

    // add all previously imported items
    const auto& mediaTypes = import.GetMediaTypes();
    for (auto& mediaTypeData : taskData->second.m_mediaTypeData)
    {
      if (std::find(mediaTypes.begin(), mediaTypes.end(), mediaTypeData.m_mediaType) != mediaTypes.end())
        importItemsRetrievalTask->SetLocalItems(mediaTypeData.m_localItems, mediaTypeData.m_mediaType);
    }

    // if processing the task failed remove the import (no cleanup needed)
    CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: starting import items retrieval task for items from {}...", import);
    if (!ProcessTask(importItemsRetrievalTask))
    {
      CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: import items retrieval task for items from {} failed", import);
      m_importTaskData.erase(taskData++);
    ...
    for (auto& mediaTypeData : taskData->second.m_mediaTypeData)
      mediaTypeData.m_importedItems = importItemsRetrievalTask->GetRetrievedItems(mediaTypeData.m_mediaType);
...
void CMediaImportTaskProcessorJob::ProcessChangesetTasks()
{
  for (auto&& taskData : m_importTaskData)
  {
    ...
    {
      CMediaImportChangesetTask* changesetTask =
        new CMediaImportChangesetTask(import, MediaImportHandlerPtr(mediaTypeData->m_importHandler->Create()),
          mediaTypeData->m_localItems, mediaTypeData->m_importedItems, taskData.second.m_partialChangeset);

      // if processing the task failed remove the import (no cleanup needed)
      CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: starting import changeset task for {} items from {}...",
        mediaTypeData->m_mediaType.c_str(), import);
      if (!ProcessTask(changesetTask))
      {
        CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: import changeset task for {} items from {} failed",
          mediaTypeData->m_mediaType.c_str(), import);
      ...
      {
        CLog::Log(LOGDEBUG, "CMediaImportTaskProcessorJob: no {} items from {} changed",
          mediaTypeData->m_mediaType.c_str(), import);
      }
...
void CMediaImportTaskProcessorJob::ProcessSynchronisationTasks()
{
  ...
  for (auto& taskData : m_importTaskData)
  {
    ...
    for (auto&& mediaTypeData = taskData.second.m_mediaTypeData.begin(); mediaTypeData != taskData.second.m_mediaTypeData.end(); ++mediaTypeData)
    {
      ...
      // if processing the task failed remove the import (no cleanup needed)
      CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: starting import synchronisation task for {} items from {}...", mediaTypeData->m_mediaType.c_str(), import);
      if (!ProcessTask(synchronisationTask))
      {
        CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: import changeset task for {} items from {} failed", mediaTypeData->m_mediaType.c_str(), import);
...
void CMediaImportTaskProcessorJob::ProcessCleanupTasks()
{
  ...
  for (auto& taskData : m_importTaskData)
  {
    ...
    for (auto&& mediaTypeData = taskData.second.m_mediaTypeData.rbegin(); mediaTypeData != taskData.second.m_mediaTypeData.rend(); ++mediaTypeData)
    {
      ...
      // if processing the task failed remove the import (no cleanup needed)
      CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: starting import cleanup task for {} items from {}...", mediaTypeData->m_mediaType.c_str(), import);
      if (!ProcessTask(cleanupTask))
        CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: import cleanup task for {} items from {} failed", mediaTypeData->m_mediaType.c_str(), import);
...
void CMediaImportTaskProcessorJob::ProcessRemovalTasks()
{
  ...
  for (auto& taskData : m_importTaskData)
  {
    ...
    for (auto&& mediaTypeData = taskData.second.m_mediaTypeData.rbegin(); mediaTypeData != taskData.second.m_mediaTypeData.rend(); ++mediaTypeData)
    {
      ...
      CLog::Log(LOGINFO, "CMediaImportTaskProcessorJob: starting import removal task for {} items from {}...", mediaTypeData->m_mediaType.c_str(), import);
      if (!ProcessTask(removalTask))
        CLog::Log(LOGWARNING, "CMediaImportTaskProcessorJob: import removal task for {} items from {} failed", mediaTypeData->m_mediaType.c_str(), import);
...
bool CMediaImportTaskProcessorJob::AddImport(const CMediaImport& import, std::vector<MediaImportTaskType> tasksToBeProcessed)
{
  ...
  for (const auto& mediaType : import.GetMediaTypes())
  {
    MediaImportTaskData::MediaTypeTaskData mediaTypeData = { mediaType, m_importHandlerManager->GetImportHandler(mediaType) };
  ...
  m_importTaskData.emplace(std::make_pair(import.GetPath(), import.GetMediaTypes()), importTaskData);
