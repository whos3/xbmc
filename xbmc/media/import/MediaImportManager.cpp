...
std::string MediaTypesToLabel(const GroupedMediaTypes& mediaTypes)
{
  ...
  {
    std::string mediaType = CMediaTypes::GetCapitalPluralLocalization(*itMediaType);
...
std::vector<std::string> MediaTypesToLabels(const MediaTypes& mediaTypes)
{
  ...
  std::vector<GroupedMediaTypes> groupedMediaTypes = CMediaImportManager::GetInstance().GetSupportedMediaTypesGrouped(mediaTypes);
  for (const auto& groupedMediaType : groupedMediaTypes)
    labels.push_back(MediaTypesToLabel(groupedMediaType));
...
bool CMediaImportManager::AddSource(const std::string& sourceID, const std::string& friendlyName, const std::string& iconUrl /* = "" */, const MediaTypes& mediaTypes /* = MediaTypes() */)
{
  return AddSourceInternal(sourceID, friendlyName, iconUrl, mediaTypes, false);
}

bool CMediaImportManager::AddSourceSync(const std::string& sourceID, const std::string& friendlyName, const std::string& iconUrl /* = "" */, const MediaTypes& mediaTypes /* = MediaTypes() */)
{
  return AddSourceInternal(sourceID, friendlyName, iconUrl, mediaTypes, true);
}

void CMediaImportManager::RegisterSource(const std::string& sourceID)
{
  if (sourceID.empty())
  {
...
bool CMediaImportManager::AddSelectiveImport(const std::string &sourceID, const std::string &path, const std::string& parentPath, const GroupedMediaTypes &mediaTypes)
{
  if (sourceID.empty() || path.empty() || parentPath.empty() || mediaTypes.empty())
  {
  ...
  return AddImport(sourceID, path, parentPath, mediaTypes, false);
}
...
bool CMediaImportManager::AddSourceInternal(const std::string& sourceID, const std::string& friendlyName, const std::string& iconUrl, const MediaTypes& mediaTypes, bool synchronous)
{
...
bool CMediaImportManager::AddImport(const std::string &sourceID, const std::string &path, const std::string& parentPath, const GroupedMediaTypes &mediaTypes, bool recursive)
{
  // check if the import already exists
  CMediaImport import;
  if (FindImport(path, mediaTypes, import))
  ...
  if (recursive)
    ...
    newImport = CMediaImport::CreateSelective(path, parentPath, mediaTypes, source);
