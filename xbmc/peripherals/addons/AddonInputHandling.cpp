...
CAddonInputHandling::CAddonInputHandling(CPeripheral* peripheral, IInputHandler* handler, IDriverReceiver* receiver)
{
  ...
  else
  {
    ...
    if (m_buttonMap->Load())
    {
      ...
      if (receiver)
      {
        m_inputReceiver.reset(new CDriverReceiving(receiver, handler, m_buttonMap.get()));
...
IInputHandler* CAddonInputHandling::GetInputHandler(void)
{
  if (m_inputReceiver)
    return static_cast<CDriverReceiving*>(m_inputReceiver.get())->GetInputHandler();
