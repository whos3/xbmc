...
ssize_t CWebServer::ContentReaderCallback(void *cls, uint64_t pos, char *buf, size_t max)
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->CanLogComponent(LOGWEBSERVER))
    s_logger->debug("[OUT] wrote %d bytes from {} in range ({} - {})", written, context->writePosition, start, end);
...
{
  CLog::Log(LOGFATAL, "[MHD] serious error: reason \"%s\" in file \"%s\" at line %ui", reason ? reason : "",
            file ? file : "", line);
...
static void logFromMHD(void* unused, const char* fmt, va_list ap)
{
  ...
  else
  {
    ...
    if (errDsc.empty())
      CLog::Log(LOGERROR, "MHD reported error with unprintable string \"{}\"", fmt);
    else
    {
...
bool CWebServer::LoadCert(std::string &skey, std::string &scert)
{
  ...
  if (file.LoadFile(keyFile, buf) > 0)
  {
    ...
    m_logger->error("{}: Error loading: %s", __FUNCTION__, keyFile);
  ...
  if (file.LoadFile(certFile, buf) > 0)
  {
    ...
    m_logger->error("%s: Error loading: %s", __FUNCTION__, certFile);

  if (!skey.empty() && !scert.empty())
  {
    m_logger->info("%s: found server key: %s, certificate: %s, HTTPS support enabled", __FUNCTION__, keyFile, certFile);
    return true;
  }
  return false;
}
