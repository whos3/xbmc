...
#include "ohUPnPMediaServerConnectionManagerService.h"
...
COhUPnPMediaServerConnectionManagerService::COhUPnPMediaServerConnectionManagerService(COhUPnPRootDevice& device)
  : IOhUPnPService(UPNP_DOMAIN_NAME, UPNP_SERVICE_TYPE_CONNECTIONMANAGER, 2),
    m_device(device),
    m_connectionManager(nullptr)
{ }

COhUPnPMediaServerConnectionManagerService::~COhUPnPMediaServerConnectionManagerService()
{
  Stop();
}

void COhUPnPMediaServerConnectionManagerService::Start()
{
...
void COhUPnPMediaServerConnectionManagerService::Stop()
{
  if (!IsRunning())
    return;
...
bool COhUPnPMediaServerConnectionManagerService::IsRunning() const
{
  return m_connectionManager != nullptr;
}

COhUPnPMediaServerConnectionManagerService::ConnectionManager::ConnectionManager(COhUPnPMediaServerConnectionManagerService& service, OpenHome::Net::DvDeviceStd& device)
    ...
    m_service(service)
{
  ...
  SetPropertySourceProtocolInfo(profile.GetProtocolInfo());
...
COhUPnPMediaServerConnectionManagerService::ConnectionManager::~ConnectionManager()
{ }

void COhUPnPMediaServerConnectionManagerService::ConnectionManager::GetProtocolInfo(OpenHome::Net::IDvInvocationStd& invocation, std::string& source, std::string& sink)
{
  ...
  if (!COhUPnPDeviceProfilesManager::GetInstance().FindProfile(clientDevice, profile))
    CLog::Log(LOGINFO, "COhUPnPMediaServerConnectionManagerService::GetProtocolInfo: client doesn't match any profiles");
  else
    CLog::Log(LOGINFO, "COhUPnPMediaServerConnectionManagerService::GetProtocolInfo: client matches profile %s", profile.GetName().c_str());

  source = profile.GetProtocolInfo();
  GetPropertySinkProtocolInfo(sink);

  if (g_advancedSettings.CanLogComponent(LOGUPNP))
    CLog::Log(LOGDEBUG, "[ohNet] --> GetProtocolInfo(): source = %s; sink = %s", source.c_str(), sink.c_str());
}

void COhUPnPMediaServerConnectionManagerService::ConnectionManager::PrepareForConnection(OpenHome::Net::IDvInvocationStd& invocation, const std::string& remoteProtocolInfo,
...
void COhUPnPMediaServerConnectionManagerService::ConnectionManager::ConnectionComplete(OpenHome::Net::IDvInvocationStd& invocation, int32_t connectionID)
{
  if (g_advancedSettings.CanLogComponent(LOGUPNP))
...
void COhUPnPMediaServerConnectionManagerService::ConnectionManager::GetCurrentConnectionIDs(OpenHome::Net::IDvInvocationStd& invocation, std::string& connectionIDs)
{
  if (g_advancedSettings.CanLogComponent(LOGUPNP))
...
void COhUPnPMediaServerConnectionManagerService::ConnectionManager::GetCurrentConnectionInfo(OpenHome::Net::IDvInvocationStd& invocation, int32_t connectionID, int32_t& rcsID,
