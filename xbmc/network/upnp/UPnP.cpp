...
public:
    ...
    virtual bool OnMSAdded(PLT_DeviceDataReference& device)
    {
        ...
        if (CSettings::GetInstance().GetBool("services.upnpimport"))
        {
          ...
          if (CMediaImportManager::GetInstance().AddSource(sourceID, device->GetFriendlyName().GetChars(), device->GetIconUrl("image/png").GetChars(), mediaTypes))
            CMediaImportManager::GetInstance().RegisterSource(sourceID);
        }
