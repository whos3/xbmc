...
std::vector<CMediaImport> CVideoDatabase::GetImports()
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("SELECT path.strPath, source.identifier, import.media_type, import.recursive, import.last_sync, import.settings, parentPath.strPath FROM import "
                        ...
                        "LEFT JOIN path AS parentPath ON path.idParentPath = parentPath.idPath");
    m_pDS->query(strSQL.c_str());
    while (!m_pDS->eof())
    {
      ...
      if (m_pDS->fv(3).get_asBool())
        ...
        import = CMediaImport::CreateSelective(m_pDS->fv(0).get_asString(), m_pDS->fv(6).get_asString(), CMediaTypes::Split(m_pDS->fv(2).get_asString()), CMediaImportSource(m_pDS->fv(1).get_asString()), lastSynced);
...
int CVideoDatabase::AddImport(const CMediaImport &import)
{
  ...
  try
  {
    ...
    if (idPath <= 0)
    {
      idPath = AddPath(import.GetPath(), import.GetParentPath());
      if (idPath <= 0)
        return -1;
    }
